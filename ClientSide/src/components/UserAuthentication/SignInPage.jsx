...
export default function SignInPage() {
    ...
    const onDataSubmit = async (data) => {
        if (isValid) {
            try {
                const response = await axios.post("http://localhost:3000/api/v1/signinUser", { data });
                        ...
                        <div className='me-6'>
                            <form className="mt-6 mx-auto" onSubmit={handleSubmit(onDataSubmit)}>
                                ...
                                <Link to="/registration">
                                    <p className='text-center cursor-pointer mb-6'>
                                        <span className='text-xs' style={{ color: "#B20303" }}>Not an Account?</span>
